  ...
  const items: Item[] = [
    {
      ...
      quote:
      "Trabalhar com a Ezpage foi uma experiência transformadora! Eles elevaram nossa marca a um novo patamar e superaram todas as expectativas.",
      name: "Jessie J",
      role: "Hotmart creator",
    },
    {
        ...
        <div className="relative pb-12 md:pb-20">
          ...
          <div className="text-center">
            ...
            <div className="flex flex-wrap justify-center -m-1.5">
              {items.map((item, index) => (
                <button
                  className={`btn-sm m-1.5 text-xs py-1.5 text-white transition duration-150 ease-in-out [background:linear-gradient(theme(colors.slate.900),_theme(colors.slate.900))_padding-box,_conic-gradient(theme(colors.slate.400),_theme(colors.slate.700)_25%,_theme(colors.slate.700)_75%,_theme(colors.slate.400)_100%)_border-box] relative before:absolute before:inset-0 before:bg-slate-800/30 before:rounded-full before:pointer-events-none ${
                    active === index
                      ? "opacity-100"
